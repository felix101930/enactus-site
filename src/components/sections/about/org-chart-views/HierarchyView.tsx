...
const StaffCard = ({ name, role, image }: { name: string, role: string, image?: string }) => (
  <div className="flex flex-col xl:flex-row items-center p-4 bg-white border border-gray-100 rounded-xl shadow-sm hover:shadow-lg transition-all">
    {image ? (
      <img src={image} alt={name} className="w-10 h-10 rounded-full object-cover mb-2 xl:mb-0 xl:mr-3 border border-gray-100" />
    ) : (
      ...
      <div className="z-10 bg-gray-900 text-white px-12 py-8 rounded-[2rem] flex gap-12 shadow-2xl">
        {data.coPresidents.map((p: any) => (
          <div key={p.name} className="flex flex-col items-center">
            {p.image ? (
              <img src={p.image} alt={p.name} className="w-16 h-16 rounded-full object-cover mb-3 border-2 border-yellow-400" />
            ) : (
      ...
      <div className="w-full max-w-6xl grid grid-cols-1 md:grid-cols-2 gap-8 mt-4 z-10">
        ...
        <div className="bg-white rounded-[2rem] p-8 shadow-lg border-t-8 border-gray-600">
          ...
          <div className="flex flex-row items-center gap-6 mb-8 pb-6 border-b border-gray-100">
            {data.operations.vp.image ? (
              <img src={data.operations.vp.image} alt={data.operations.vp.name} className="w-16 h-16 rounded-full object-cover shrink-0 border-2 border-gray-600" />
            ) : (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
            {data.operations.directors.map((d: any) => (
              <StaffCard key={d.name} name={d.name} role={d.role} image={d.image} />
            ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-[2rem] p-8 shadow-lg border-t-8 border-yellow-400">
          {/* Header Block: Fixed alignment to prevent indentation */}
          <div className="flex flex-row items-center gap-6 mb-8 pb-6 border-b border-gray-100">
            {data.projects.vp.image ? (
              <img src={data.projects.vp.image} alt={data.projects.vp.name} className="w-16 h-16 rounded-full object-cover shrink-0 border-2 border-yellow-400" />
            ) : (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
            {data.projects.managers.map((m: any) => (
              <StaffCard key={m.name} name={m.name} role={m.role} image={m.image} />
            ))}
          </div>
        </div>
